...
This module keeps `src/app.py` as a tiny launcher so developers can
import the GUI class from `src/gui.py` in tests or interactive sessions.
"""
...
# Import the GUI module with fallbacks so the launcher can be executed as:
#  - python -m src.app (package)
#  - python ./src/app.py   (script from project root)
#  - python ./src/app.py   (when cwd == src/)
try:
    # when used as a package
    from .gui import AstridsEyeGUI
except Exception:
    # Ensure project root and src directory are on sys.path so imports succeed
    ...
    try:
        # try importing the module as local `gui` (when src/ is on sys.path)
        from gui import AstridsEyeGUI
    except Exception:
        # try package-style import (when project_root is on sys.path)
        from src.gui import AstridsEyeGUI


def main():
    root = tk.Tk()
    app = AstridsEyeGUI(root)
    root.mainloop()
